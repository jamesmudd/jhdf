...
public class TreeRead {
	...
	private static void printGroup(RandomAccessFile raf, int sizeOfOffsets, int sizeOfLengths, int leafK,
			int internalK, SymbolTableEntry ste, int level) {
		
		level++;

		BTreeNode rootbTreeNode = new BTreeNode(raf, ste.getBTreeAddress(), sizeOfOffsets, sizeOfLengths, leafK, internalK);
