...
def write_chunked_datasets(f):
    ...
    float_group.create_dataset('float64', data=data, dtype='f8', chunks=(3, 4), shuffle=True, compression="gzip",
                               compression_opts=9)

    int_group = f.create_group('int')
    int_group.create_dataset('int8', data=data, dtype='i1', chunks=(5, 3), shuffle=True, compression="gzip")
    int_group.create_dataset('int16', data=data, dtype='i2', chunks=(1, 1), shuffle=True, compression="gzip",
                             compression_opts=1)
    int_group.create_dataset('int32', data=data, dtype='i4', chunks=(1, 3), shuffle=True, compression="gzip",
                             compression_opts=7)
